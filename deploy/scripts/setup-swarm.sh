...
spec:
  ...
  ports:
    ...
    - protocol: TCP
      ...
      port: 8222
EOF
[ $? -eq 0 ] && log "nats manifest created" \
  || log 1 "Failed to create nats manifest"
...
cat <<EOF > ${homedir}/conf/cluster.json
{
  "cluster": {
    "name": "swarm",
    "port": 6222,
...
cat <<EOF > ${homedir}/conf/auth.json
{
  "authorization": {
    "users": [
      {
...
kubectl -n $kube_ns create cm nats-config \
  ...
  --from-file ${homedir}/conf/auth.json -o yaml \
...
mv ${homedir}/conf/cluster.json ${homedir}/debug/cluster.json
mv ${homedir}/conf/auth.json ${homedir}/debug/auth.json

kubectl -n $kube_ns delete po -l app=nats 2>&1 | tee -a $log
wait_for_nats
log "nats server configuration reloaded. swarm node ready"
exit 0
