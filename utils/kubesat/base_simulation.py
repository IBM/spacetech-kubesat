...
class BaseSimulation():
                ...
                async def callback_wrapper(msg):
                    ...
                    try:
                        ...
                        if not validate_json(shared_storage, self._schema):
                            raise ValueError("Invalid change in shared storage")
    ...
    def run(self, nats_host="nats", nats_port="4222", nats_user=None, nats_password=None, api_host="127.0.0.1", api_port=8000, redis_host="redis", redis_port=6379, redis_password=None):
        ...
        self.redis_client = RedisHandler(self.service_type, self._schema, host=self.redis_host, port=self.redis_port, password=self.redis_password)
        ...
        async def run():
            ...
            self.nats_client = NatsHandler("default", host=self.nats_host, port=self.nats_port, user=self.nats_user, password=self.nats_password, api_host=self.api_host, api_port=self.api_port, loop=asyncio.get_running_loop())
            await self.nats_client.connect()

            # creating logger
            self._logger = NatsLoggerFactory.get_logger(self.nats_client, self.service_type)
            ...
            if self.config_path is not None:
                ...
                print(f"Successfully initialized {self.sender_id} {self.service_type} from file")
            else:
                try:
                    # requesting a config from the config service
                    message = self.nats_client.create_message(self.service_type, MessageSchemas.SERVICE_TYPE_MESSAGE)
                    print(f"Requesting config from config service for node {self.service_type}")
                    ...
                    self.redis_client.set_sender_id(self.sender_id)
                    self.redis_client.set_shared_storage(self.shared_storage)
                    print(f"Successfully initialized {self.sender_id} {self.service_type} from config service")
                except:
                    try:
                        ...
                        print(f"Successfully initialized {self.sender_id} {self.service_type} from redis")
                    except Exception as e:
                        raise ValueError(f"Failed to initialize from redis. Aborting. Error: {e}")
        ...
        # run application
        uvicorn.run(self._api, host="0.0.0.0", port=self.api_port, debug=False, log_level='error')